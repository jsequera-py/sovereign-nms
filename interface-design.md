...
4. **`discovery_run.started_at` populated server-side at request receipt**
   ...
   missing cycles.
5. **Interface churn — resolved 2026-08-24, ahead of any screen.** Container
   plumbing (`veth<hex>`, `br-<hex>`, `docker<n>`) is dropped server-side in
   `_iface_view()` by `store.is_ignored_ifname()` (`c742b30`), so screens 3
   and 4 can show interface status without a flapping red dot that is purely
   an artifact of our own bookkeeping. Measured after the fix: 226 active /
   0 stale across two consecutive polls. What the UI must still not assume is
   that `state = 'active'` means *recently confirmed* — that is §4's job, and
   §4 reads evidence age, not interface state.
